    ...
    return (
      ...
      >
        <div className="w-full max-w-md px-8">
          <div className="text-[#00f0d4] text-xs space-y-1">
            {bootLines.map((line, i) => (
              <div
                key={i}
                style={{
                  color: line.startsWith("✓") ? "#00f0d4" : "#555",
                  textShadow: line.startsWith("✓") ? "0 0 8px rgba(0,240,212,0.4)" : "none",
                }}
              >
                {line}
              </div>
            ...
            <span
              ...
              style={{ backgroundColor: "#00f0d4", animation: "blink 1s step-end infinite" }}
            />
          </div>
        </div>
      </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundImage:
          "linear-gradient(rgba(0,240,212,0.015) 1px, transparent 1px), linear-gradient(90deg, rgba(0,240,212,0.015) 1px, transparent 1px)",
    ...
    >
      ...
      {/* Desktop area */}
      <div ref={desktopRef} className="absolute inset-0 pb-12">
        {/* Desktop icons */}
        {DESKTOP_ICONS.map(({ app, x, y }) => (
          <button
            key={app}
            className="absolute flex flex-col items-center gap-1.5 p-2 rounded hover:bg-white/5 transition-colors cursor-pointer"
            style={{ left: x, top: y }}
            onDoubleClick={() => openApp(app)}
          >
            <div
              className="w-12 h-12 flex items-center justify-center border border-[#1a1a1a] bg-[#0f0f0f] text-lg"
              style={{
                color: app === "terminal" ? "#00f0d4" : app === "monitor" ? "#f0a500" : "#888",
                boxShadow: app === "terminal" ? "0 0 12px rgba(0,240,212,0.1)" : "none",
              }}
            >
              {APPS[app].icon}
            </div>
            <span className="text-[10px] text-[#888] text-center leading-tight w-16">
              {APPS[app].label}
            </span>
          </button>
        ))}
  ...
  return (
    ...
    >
      <div
        ...
        style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.6), 0 0 1px rgba(0,240,212,0.1)" }}
      >
        {/* Title bar */}
        <div
      ...
      <div
        ...
        style={{
          background: "linear-gradient(135deg, transparent 50%, rgba(0,240,212,0.2) 50%)",
        }}
      />
    </div>
  );
...
function TerminalApp() {
  ...
  const run = (cmd: string) => {
    ...
    const add = (lines: { text: string; color: string }[]) =>
      setHistory((prev) => [...prev, { text: prompt + cmd, color: "#00f0d4" }, ...lines]);
    ...
    if (command === "exit") {
      add([{ text: "There is no escape from JC OS. 😈", color: "#f0a500" }]);
      return;
    }

    if (command === "neofetch") {
      add([
        { text: "       ___   ____", color: "#00f0d4" },
        { text: "      | |   / ___|", color: "#00f0d4" },
        { text: "   _  | |  | |    ", color: "#00f0d4" },
        { text: "  | |_| |  | |___ ", color: "#f0a500" },
        { text: "   \\___/    \\____|", color: "#f0a500" },
        { text: "", color: "#888" },
        { text: "  root@jc-os", color: "#00f0d4" },
        { text: "  ─────────────────", color: "#333" },
        ...
        { text: "  Memory: Coffee / Unlimited", color: "#f0a500" },
      ]);
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center text-[11px] leading-[1.8]">
        <span className="text-[#00f0d4] shrink-0">{prompt}&nbsp;</span>
        <input
          ref={inputRef}
          type="text"
          value={input}
          ...
          spellCheck={false}
          autoComplete="off"
          className="flex-1 bg-transparent outline-none text-[#e8e8e8] caret-[#00f0d4] text-[11px]"
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-3 py-2 border-b border-[#1a1a1a] bg-[#080808] flex items-center gap-2 text-[10px] shrink-0">
        <button
          ...
          className="text-[#555] hover:text-[#00f0d4] transition cursor-pointer"
        >
          ←
        </button>
          ...
          {Object.entries(entries).map(([name, entry]) => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-2 py-1.5 text-[11px] text-left hover:bg-white/5 transition rounded cursor-pointer ${
                selectedFile === name ? "bg-[#00f0d4]/10 text-[#00f0d4]" : "text-[#888]"
              }`}
            ...
            >
              <span style={{ color: entry.type === "dir" ? "#f0a500" : "#555" }}>
                {entry.type === "dir" ? "📁" : "📄"}
              </span>
              {name}
            </button>
        ...
        <div>
          ...
          <div className="h-4 bg-[#111] overflow-hidden border border-[#1a1a1a]">
            <div
              ...
              style={{
                width: `${cpu}%`,
                background: cpu > 80 ? "#ff5f57" : "linear-gradient(90deg, #00f0d4, #f0a500)",
              }}
            />
          </div>
        ...
        <div className="border border-[#1a1a1a]">
          ...
          {processes.map((p) => (
            ...
            >
              ...
              <span style={{ color: p.cpu > 50 ? "#f0a500" : p.cpu > 80 ? "#ff5f57" : "#555" }}>
                {p.cpu}
              </span>
    ...
    <div className="h-full flex flex-col items-center justify-center p-8 text-center">
      ...
      <svg width="64" height="42" viewBox="0 0 120 80" fill="none" className="mb-4">
        <path
          ...
          fill="none"
          style={{ filter: "drop-shadow(0 0 6px #00f0d4)" }}
        />
        <path
        ...
        />
        <circle cx="50" cy="40" r="3.5" fill="#00f0d4" style={{ filter: "drop-shadow(0 0 4px #00f0d4)" }} />
      </svg>
      ...
      <div className="space-y-1.5 text-[11px] text-[#888]">
        <div>
          Built by{" "}
          <span className="text-[#00f0d4]" style={{ textShadow: "0 0 8px rgba(0,240,212,0.3)" }}>
            Jacob Chodubski
          </span>
        </div>
    ...
    >
      ...
      >
        <svg width="16" height="11" viewBox="0 0 120 80" fill="none">
          <path d="M20 10 L20 50 Q20 65 35 65 L42 65" stroke="#00f0d4" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" fill="none" />
          <path d="M100 15 L75 15 Q60 15 60 30 L60 50 Q60 65 75 65 L100 65" stroke="#f0a500" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" fill="none" />
          <circle cx="50" cy="40" r="4" fill="#00f0d4" />
        </svg>
      </button>
